...
public class ParentsRepository implements ChildrenDataSource {
     ...
     * @return
     */
    public static ParentsRepository getInstance(@NonNull AppExecutors appExecutors, @NonNull Context context){
        if(INSTANCE == null){
            synchronized (ParentsRepository.class){
                if(INSTANCE == null){
                    INSTANCE = new ParentsRepository(appExecutors, context);
                }
            }
        }
        return INSTANCE;
